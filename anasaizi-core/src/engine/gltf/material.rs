use crate::engine::{RenderContext, GlTFPBRMeshPushConstants};
...
use crate::math::{Vector4, Vector3, Matrix4, Vector2};
use crate::vulkan::ShaderFlags;
use gltf::texture::Sampler;

impl GlTFPBRMeshPushConstants {
    pub(crate) fn from_gltf(g_material: gltf::Material<'_>, imp: &ImportData) -> Self {
        ...
        if g_material.alpha_mode() != gltf::material::AlphaMode::Opaque {
            ...
            if g_material.alpha_mode() == gltf::material::AlphaMode::Mask && g_material.alpha_cutoff().is_some() {
                alpha_cutoff = g_material.alpha_cutoff().unwrap();
            }
        }

        let mut material = GlTFPBRMeshPushConstants {
            ...
            metallic_roughness_values: Vector4::new(pbr.metallic_factor(),pbr.roughness_factor(), 0.0, 0.0),
